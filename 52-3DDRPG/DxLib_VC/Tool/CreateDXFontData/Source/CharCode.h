...
#define CODEPAGE_UHC			(949)
#define CODEPAGE_BIG5			(950)
#define CODEPAGE_UTF16LE		(1200)
#define CODEPAGE_UTF16BE		(1201)
#define CODEPAGE_ASCII			(1252)
#define CODEPAGE_UTF8			(65001)
#define CODEPAGE_UTF32LE		(32766)			// 正式なコードが無かったので勝手に定義
#define CODEPAGE_UTF32BE		(32767)			// 正式なコードが無かったので勝手に定義
...
#define WCHAR_T_CODEPAGE				( sizeof( wchar_t ) > 2 ? DX_CODEPAGE_UTF32LE : DX_CODEPAGE_UTF16LE )
...
extern	u32				GetCharCode(			const char *CharCode, int CodePage, int *CharBytes ) ;					// １文字の文字コードと文字のバイト数を取得する
extern	int				PutCharCode(			u32 CharCode, int CodePage, char *Dest ) ;							// 文字コードを通常の文字列に変換する、終端にヌル文字は書き込まない( 戻り値：書き込んだバイト数 )
extern	u32				ConvCharCode(			u32 SrcCharCode, int SrcCodePage, int DestCodePage ) ;				// 文字コードを指定のコードページの文字に変換する
extern	int				ConvCharCodeString(		const u32 *Src, int SrcCodePage, u32 *Dest, int DestCodePage ) ;	// １文字４バイトの配列を、別コードページの１文字４バイトの配列に変換する( 戻り値：変換後のサイズ、ヌル文字含む( 単位：バイト ) )
extern	int				StringToCharCodeString( const char  *Src, int SrcCodePage, u32  *Dest ) ;						// 文字列を１文字４バイトの配列に変換する( 戻り値：変換後のサイズ、ヌル文字含む( 単位：バイト ) )
extern	int				CharCodeStringToString( const u32 *Src, char *Dest, int DestCodePage ) ;						// １文字４バイトの配列を文字列に変換する( 戻り値：変換後のサイズ、ヌル文字含む( 単位：バイト ) )
...
extern	u32				GetStringCharCode(		const char *String, int CodePage, int Index ) ;							// 指定番号の文字のコードを取得する
